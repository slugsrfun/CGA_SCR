...
Some individuals are poorly sequenced (see outliers in the plot above). We will remove samples from the dataset for SCR analyses if they are not well genotyped (GTseek samples with <100 genotyped loci, LGC samples with <30% genotyped loci). Those samples will also be removed from consideration for the population genetics analyses, as we can not verify that they constitute distinct individuals. Below is the full range of missing data and the distribution for samples that are filtered...  
```{r dropInds}
#Plot of levels of missing data across all loci...
hist(allmiss$F_MISS, main = "Proportion of missing data", 
     xlab = "Missing Data", ylab = "Samples")
abline(v = 0.7, lwd = 2, col = "red")
legend("top", text.col="red", 
       legend=paste("n =", sum(allmiss$F_MISS > 0.7), "with >70% missing"))
...
```
Filtering samples that were genotyped at fewer than 100 SNPs (GTSeek) or 30% of all loci (LGC) from the set used for recapture analyses resulted in `r sum(sampleMetadata$FILT)` samples being removed from the dataset.  
