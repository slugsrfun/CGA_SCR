...
In this markdown, we will revisit predictions from these simulations and use data for *known* female mortalities (which likely underestimates total mortality) to address two questions:

i. Which scenario considered in the PVA from Hooker et al. (2020) is most similar to the recent past in central GA?
ii. Which scenario do estimates of abundance from the present study (see `13_fitHooker_bothSexes`) most closely align with?  
...
ii. Refit the Hooker et al. (2020) model to the capture history from that study (using scripts and data from the project's [GitHub repository](https://github.com/rbchan/ga-bear-pva))
...
iv. Compare abundance forecasts from scenarios considered in Hooker et al. (2020) to posterior distributions of female abundance from the SCR model fit in this study
...
From the quick comparisons above, it appears that total mortality is indeed higher in males. However, legal harvest numbers are higher in females. The difference in harvest is more than offset by higher male mortality rates from vehicular collisions.  
...
```{r redoForc}
#Turn of warning messages for the forecasting
options(warn=-1)
...
#Load output from simulated scenarios
load("forc-h0.gzip")
load("forc-h5.gzip")
load("forc-h10.gzip")
load("forc-h15.gzip")
...
load("../scr/samples-bothSexes.gzip")
...
Where do our point estimates fall in the distribution of median abundances from PVA simulations? In other words, what percentile is associated with the medians of our posterior distributions for abundance?  
```{r percCalcs}
...
From this comparison, we can see that the point estimate from 2023 is inconsistent with the more limited harvest rates of $h = 0$ (*status quo*) and $h = 5$. However, it falls within the central 95th percentile interval (between the 2.5th and 97.5th percentile) for all other scenarios. Meanwhile, the point esimate of female abundance from 2024 falls within this interval for all simulated scenarios. This suggests that our estimates of abundance are broadly consistent with predictions from PVA simulations, particularly in the second year of the study.  

### Quantifying "fit" of the scenarios
Now to assess whether we can distinguish between the "fit" of the different scenarios simulated in Hooker et al. (2020) to our abundance estimates. Notably, there is substantial overlap in the predictions from the model with more limited harvest rates, partially due to the assumption of Poisson-distributed annual harvest (*i.e.*, with $h=5$, we are not removing exactly 5 bears from the population each year). Annual known mortality numbers since the Hooker et al. (2020) study were most similar to the *status quo* or 5 additional females removed scenarios (see above). However, we are still interested in assessing whether the different scenarios generate distinguishable predictions for abundance.  

To assess the fit of our estimates (*i.e.*, posterior distributions of female abundance in 2023 and 2024) to the management scenarios from Hooker et al. (2020), we will calculate Kullback-Leibler divergence ($D_{kl}$) between the distributions of simulated abundance under each scenario and the posterior distribution of estimated abundance for each of the two study years.  

$$D_{kl} = \sum_i P_i \log(\frac{P_i}{Q_i})$$

where the sum is calculated over the range of abundance values in the posterior distribution and forecasted abundances, $P_i$ is the probability density of value $i$ in the posterior distribution, and $Q_i$ is the probability density of value $i$ in the distribution of forecasted abundances. As these distributions do not overlap perfectly for all scenarios, we also add $\epsilon = 1\times10^{-16}$ to all counts when calculating $P_i$ and $Q_i$.  

Here's the function we'll use to calculate $D_{kl}$.  
```{r KLDfxn}
#a function to calculate KL divergence with an epsilon added to avoid P=0 | Q=0
KLD_eps <- function(true=NULL, approx=NULL, eps=1e-16) {
  
  get_probs <- function(x, Xmin, Xmax, eps=1e-16) {
    x_factor <- factor(x, levels=Xmin:Xmax)
    x_table <- table(x_factor)+eps
    X_probs <- x_table/sum(x_table)
    return(X_probs)
  }
  
  P <- get_probs(true, min(true, approx), max(true, approx))
  Q <- get_probs(approx, min(true, approx), max(true, approx))
  
  KLD <- sum(P*(log(P)-log(Q)))
  
  return(KLD)
}
```

Now we use this function to calculate $D_{kl}$ for each of the two years (2023 and 2024) under each of the five simulated scenarios.  
```{r KLD}
#Load our posterior distribution of abundances
load("../scr/samples-bothSexes.gzip")
out <- coda::as.mcmc.list(samples)
true23 <- unlist(out[,"Nf[1]"])
true24 <- unlist(out[,"Nf[2]"])

#####
#h=0 (status quo)
ap23 <- as.vector(forc.h0$Nbar[which(fyrs == 2023),,])
ap24 <- as.vector(forc.h0$Nbar[which(fyrs == 2024),,])

(KLD.h0.23 <- KLD_eps(true=tr23, approx=ap23))
(KLD.h0.24 <- KLD_eps(true=tr24, approx=ap24))
(KLD.h0 <- KLD.h0.23 + KLD.h0.24)


#####
#h=5
ap23 <- as.vector(forc.h5$Nbar[which(fyrs == 2023),,])
ap24 <- as.vector(forc.h5$Nbar[which(fyrs == 2024),,])

(KLD.h5.23 <- KLD_eps(true=tr23, approx=ap23))
(KLD.h5.24 <- KLD_eps(true=tr24, approx=ap24))
(KLD.h5 <- KLD.h5.23 + KLD.h5.24)


#####
#h=10
ap23 <- as.vector(forc.h10$Nbar[which(fyrs == 2023),,])
ap24 <- as.vector(forc.h10$Nbar[which(fyrs == 2024),,])

(KLD.h10.23 <- KLD_eps(true=tr23, approx=ap23))
(KLD.h10.24 <- KLD_eps(true=tr24, approx=ap24))
(KLD.h10 <- KLD.h10.23 + KLD.h10.24)


#####
#h=15
ap23 <- as.vector(forc.h15$Nbar[which(fyrs == 2023),,])
ap24 <- as.vector(forc.h15$Nbar[which(fyrs == 2024),,])

(KLD.h15.23 <- KLD_eps(true=tr23, approx=ap23))
(KLD.h15.24 <- KLD_eps(true=tr24, approx=ap24))
(KLD.h15 <- KLD.h15.23 + KLD.h15.24)


#####
#h=20
ap23 <- as.vector(forc.h20$Nbar[which(fyrs == 2023),,])
ap24 <- as.vector(forc.h20$Nbar[which(fyrs == 2024),,])

(KLD.h20.23 <- KLD_eps(true=tr23, approx=ap23))
(KLD.h20.24 <- KLD_eps(true=tr24, approx=ap24))
(KLD.h20 <- KLD.h20.23 + KLD.h20.24)

```

Note that the scenario that most closely matches the posterior of abundance differs between years. Combining $D_{kl}$ across years suggests that $h=15$ most closely mathces abundance posteriors, while $h=20$ and $h=10$ most closely match abundance estimates from 2023 and 2024, respectively.  

```{r makeKable}
out <- data.frame(Year1 = 
                    c(KLD.h0.23, KLD.h5.23, KLD.h10.23, KLD.h15.23, KLD.h20.23),
                  Year2 =
                    c(KLD.h0.24, KLD.h5.24, KLD.h10.24, KLD.h15.24, KLD.h20.24),
                  Combined = 
                    c(KLD.h0, KLD.h5, KLD.h10, KLD.h15, KLD.h20))
rownames(out) <- c("Status Quo", "h = 5", "h = 10", "h = 15", "h = 20")
