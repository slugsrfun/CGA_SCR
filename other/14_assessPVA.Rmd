...
To compare our abundance estimates to the predictions from simulations in Hooker et al. (2020), we first need to characterize the average total mortalities for a few different periods. This way we can identify the scenario that most closely matches the known mortality data since the period covered by the Hooker et al. (2020) analysis.  
...
Now we'll look at the distribution of projected population sizes under each of the management alternatives (*status quo*, +5, +10, +15, or +20 additional females harvested) and ask where our estimates of female abundance in 2023 (138) and 2024 (166) fall in those distributions.  
```{r compareDist}
load("forc-h0.gzip")
load("forc-h5.gzip")
load("forc-h10.gzip")
...
### Visual comparisons
**UPDATE CODE WITH BEST-FITTING MODEL**
...
load("../scr/samples-bothSexes-v1.gzip")
...
**UPDATE THIS WITH RESULTS FOR ALL SCENARIOS - ADD 15 & 20**  
The estimate for 2023 falls in the lower tail of the distributions of forecasted abundances under *status quo* management and with 5 additional females harvested per year, but is in the central 50th percentile interval with 10 additional females harvested per year. Meanwhile, the 2024 estimate falls in the central 50th percentile interval under all three scenarios.  

### Quantifying "fit" of the scenarios
**UPDATE CODE WITH BEST-FITTING MODEL**
Can we distinguish between the "fit" of the five scenarios simulated in Hooker et al. (2020)? There is substantial overlap in the predictions from the model, partially due to the assumption of Poisson-distributed annual harvest (*i.e.*, we are not removing exactly 5, 10, 15, or 20 bears from the population each year). Notably, annual known mortality numbers since the Hooker et al. (2020) study were most similar to the *status quo* or 5 additional females removed scenarios (see above). However, we are still interested in assessing whether these scenarios generate distinguishable predictions for abundance after < 10 years of management.  

To assess the fit of our estimates (*i.e.*, posterior distributions of female abundance in 2023 and 2024) to the management scenarios from Hooker et al. (2020), we will calculate Kullback-Leibler divergence ($D_{kl}$) between the distributions of simulated abundance under each scenario and the posterior distribution of estimated abundance for each of the two study years. We'll use the `kld_est_nn()` function from the `kldest` R package ([Hartung 2024](https://CRAN.R-project.org/package=kldest)) to estimate KL divergence between our forecasts and posteriors with a nearest-neighbor approach.  
```{r KLD}
...
# these will be the distributions that we compare in terms of 
# their ability to approximate the posterior distributions
...
From this comparison, the Kullback-Leibler divergence $D_{kl}$ is smallest for the +#### females harvested management scenario.  


## Summary
**UPDATE THIS**
Point estimates of abundance from our SCR study are broadly consistent with predictions from PVA simulations with limited harvest scenarios from Hooker et al. (2020). Point estimates of female abundance fell within the distribution of simulated abundances for 2023 and 2024 for **all scenarios (true??)**. The distribution of simulated abundances from the scenario with 5 additional females harvested per year matches the posterior distributions of female abundance in 2023 and 2024 most closely, as measured by $D_{kl}$.  
