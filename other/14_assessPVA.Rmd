...
For the latter comparison, we'll use estimates from the single $\gamma$ model, which estimates more realistic increases in female abundance between 2023 and 2024.  
...
iii. Assess predictions from the PVA in Hooker et al. (2020), given female abundance estimates from our model (`13_fitHooker_bothSexes`) and observed female mortaly data
...
### Visual comparisons
Now plot the full time series with our abundance estimates included. We focus on the v1 model, with a single recruitment rate parameter shared by males and females, as the estimates of $\gamma$ are more reliastic under this model.  
```{r plotCasts}
library(scales)
