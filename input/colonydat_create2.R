colonydat_create2 <- function(proj.name = NA, out.name = NA, moms = NA, dads = NA, kids, mar.pref="SNP@", mar.typ = "0@",
  err1="0.001@", err2="0.001@", update.alfs = 1, spp.type = 2, inbreeding = 1, ploidy = 0, fem.gamy = 0, mal.gamy = 0,
  clone = 1, sib.scale = 1, sib.prior = 0, known.alfs = 0, run.number = 1, run.length = 2, monitor = 0, windows.version = 0, 
  full.likelihood = 1, likelihood.precision = 2, prob.mom = 0, prob.dad = 0,output_file = "colony2.dat"){
  ...
  monitor.interval <- paste(10000,"! Monitor interval in Iterate# / in seconds",sep = "\t")
  ...
  cat(mar.pref, file=output_file, append=T)
  cat("\n", file=output_file, append=T)
  cat(mar.typ, file=output_file, append=T)
  cat("\n", file=output_file, append=T)
  cat(err1, file=output_file, append=T)
  cat("\n", file=output_file, append=T)
  cat(err2, file=output_file, append=T)
  cat("\n",file = output_file,append = T)
  cat("\n",file = output_file,append = T)
