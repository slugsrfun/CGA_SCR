...
## Females
In the plots that follow, all detections for individuals with spatial recaptures are plotted on the same map. Primary sampling occasions (years) are shown in different colors, while secondary sampling occasions (weeks) are plotted with different symbols. In cases where the same individual is detected at multiple hair snares *during the same week*, points are connected with a dotted line (colored to indicate the year of the movement). Each plot also gives the total number of detections for a bear, and the number of hair snares $J$, weeks $K$, and years $T$ where the individual was detected.  
...
The average distance between yearly centroids for females with spatial recaptures was `r round(mean(dist.centroid[which(is.spatrecap == TRUE)], na.rm = TRUE), 1)` m.  Among the `r sum(is.spatrecap)` females with spatial recaptures, `r sum(sr.max.within >= sr.max.all)` had their greatest distance between two detections *within* a year.  
...
The average distance between yearly centroids for males with spatial recaptures was `r round(mean(dist.centroid[which(is.spatrecap == TRUE)], na.rm = TRUE), 1)` m.  Among the `r sum(is.spatrecap)` males with spatial recaptures, `r sum(sr.max.within >= sr.max.all)` had their greatest distance between two detections *within* a year.  
...
## Digging in...
### Female clone 7
Looking more closely at the 1st female plotted (clone #7), which had a large within-week movement inferred during week 7 of 2024...  
```{r checkFem1}
...
kingrel$kinship[which(kingrel$id1 %in% Fsamps$vcfID[which(Fsamps$COLclone == 282)] & kingrel$id2 %in% Fsamps$vcfID[which(Fsamps$COLclone == 282)])]
...
The distant sample in this case was identified as a recapture by COLONY, not PLINK only, but the sample did have fewer SNPs genotyped. Still well enough to make a reliable inference here, especially given the number of samples identified as part of this clone. It seems unlikely that this is a false-positive recapture.  
...
points(locs[rownames(locs) %in% Fsamps$utmSnare[which(Fsamps$cloneID == 43 & Fsamps$snare == "HS 126")],], pch = 19)
...
kingrel <- read.table("recapPLINK.kin0", header = FALSE)
colnames(kingrel) <- c("id1","id2","nsnp","hehet","ibs0","kinship")
kingrel$kinship[which(kingrel$id1 %in% Fsamps$vcfID[which(Fsamps$COLclone == 296)] & kingrel$id2 %in% Fsamps$vcfID[which(Fsamps$COLclone == 296)])]

#Samples assigned to the same original clone by COLONY
Fsamps[which(Fsamps$COLclone == 296),]
Fsamps[which(Fsamps$COLclone == 43),]
...
#Did this clone get split by genotyping service?
table(Fsamps$GTservice[which(Fsamps$COLclone == 43)])
table(Fsamps$GTservice[which(Fsamps$COLclone == 296)])
#Yes!

```

So the recapture inference in this case is from `PLINK` only, not `COLONY`, and the sample was genotyped by GTseek, so it has fewer SNP loci available to make the match. The sample is well-genotyped, though. It appears that `COLONY` split detections of this individual by dataset - LGC and GTseek. GTseek detections were predominantly from the same hair snares (HS 066 and HS 068) as the detections in the LGC data. Given the uniformly high kinship coefficients among members of these two `COLONY` clones, it seems unlikely that the detection at HS 126 was a false-positive recapture.  
