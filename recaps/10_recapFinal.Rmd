...
In this markdown, we'll combine the results from `COLONY` and `PLINK2` to identify recaptures across the samples processed by RAPiD Genomics / LGC and GTSeek. We'll also estimate naive genotyping error rates, assess kinship coefficients between technical replicates, evaluate concordance in sex inferences from genotypic data, assign clone IDs for all samples, and assign final sexes for all unique genotypes.    
...
# 1.15% with only "good" pairs
mean(repl.out$errorRate[which(repl.out$is.cloneCOLONY==TRUE & 
                                repl.out$GTservice == "LGC")])
# 1.92% for LGC matching samples
mean(repl.out$errorRate[which(repl.out$is.cloneCOLONY==TRUE & 
                                repl.out$GTservice == "GTSeek")])
# again <0.1% for GTSeek
```

Error rates are 1-3% for the capture-seq (LGC) dataset, but substantially lower for GT-seq (GTSeek) data.  

The interesting portions of the table comparing technical replicates...  
\tiny
```{r repl.out}
repl.out[,c("ugaID","nSNPgt1","nSNPgt2","filt1","filt2","nSNPshared",
...
#Replicated samples that would not meet this threshold...
repl.out[which(repl.out$kinship < 0.45),]
```
...
Removing these samples from the dataset: 

i. `35-SGA` - high error rate in replicated sample (24%), not identified as a match by `COLONY` *or* `PLINK2`. Unsure which genotype is accurate so better to drop them both.  
ii. `227-23` - error rate is high again (14%), sample not identified as a match by `COLONY`. Kinship estimates are high, but the lowest of all pairs of replicated samples that were identified by `PLINK` as recaptures. Note that this pair of samples showed up in our `sexSamples` markdown too. One replicate is inferred to be male, the other inferred to be female. Best to exclude both samples. 

Meanwhile, we should keep the match from `PLINK` for `935-23` and `151-24` and their replicates. They match at all 122 and 117 shared SNP loci!  
```{r recapReps}
table(repl.out$is.cloneCOLONY)
table(repl.out$is.clonePLINK)
repl.out[which(repl.out$is.cloneCOLONY == FALSE),]
...
#Clone 14 - 32 samples, 28 female, 2 male, 2 undet - no direct capture to confirm majority
...
#Clone 254 - 9 samples, 8 male, 1 female - no direct capture to confirm majority
table(smd$sexGT[smd$vcfID %in% strsplit(col.out[col.out$CloneIndex == 254,3], ",")[[1]]])
smd$knownSEX[smd$vcfID %in% strsplit(col.out[col.out$CloneIndex == 254,3], ",")[[1]]]

#Clone 270 - 28 samples, 25 male, 1 female, 2 undet - no direct capture to confirm majority
table(smd$sexGT[smd$vcfID %in% strsplit(col.out[col.out$CloneIndex == 270,3], ",")[[1]]])
smd$knownSEX[smd$vcfID %in% strsplit(col.out[col.out$CloneIndex == 270,3], ",")[[1]]]
```
...
smd$region[smd$vcfID %in% strsplit(col.out[col.out$CloneIndex==61,3],",")[[1]]]
#Not really a separate region, just a historic hair sample (from 2012)

smd$region[smd$vcfID %in% strsplit(col.out[col.out$CloneIndex==296,3],",")[[1]]]
#Not really a separate region, just a historic hair sample (from 2012)
```
...
table(smd$knownSEX[smd$vcfID %in% strsplit(col.out[which(col.out$CloneIndex == 30),3],",")[[1]]])
#Clone 45 - seen above, capture agrees with majority of samples
table(smd$knownSEX[smd$vcfID %in% strsplit(col.out[which(col.out$CloneIndex == 45),3],",")[[1]]])
#Clone 144 - noted previously, cub recorded as M in the field, genotype looks F
smd$ugaID[smd$vcfID %in% strsplit(col.out[which(col.out$CloneIndex == 144),3],",")[[1]]]
```
...
However, based on our replicated genotypes, we will increase this threshold when combining inferences from `PLINK` and `COLONY` (to a mean kinship - across samples within clones - of at least 0.45).    
...
hist(compare.out$nsnp[which(compare.out$id1 %in% smd$vcfID[which(smd$GTservice == "GTSeek")] &
     ...
     main = "Overlap in SNP loci for comparisons between LGC and GTSeek samples", 
...
The filters for LGC samples (at least 30% of SNPs genotyped) keep us from matching samples based on *very* few loci.  
...
```{r whoMoved}
#Clones that were the recipients of other clones
...
#Spot checking moved clones... top 3 recipients
spotClones <- names(sort(table(finalCloneDF$final), decreasing = TRUE))[1:3]

cloneInQ <- spotClones[1]
...
#Clone 8 
cloneInQ <- 8
table(smd$sexGT[smd$finalClone == cloneInQ])
table(smd$knownSEX[smd$finalClone == cloneInQ])
#Kinship values for samples assigned to different clones by COLONY
quantile(compare.out$kinship[
  compare.out$colclone1 %in% smd$COLclone[which(smd$finalClone == cloneInQ)] & 
    compare.out$colclone2 %in% smd$COLclone[which(smd$finalClone == cloneInQ)] & 
    compare.out$iscloneCOL==FALSE])
...
There's little ambiguity in any of these cases.  
...
#Not really 2 regions, just historic sample
smd$region[which(smd$finalClone == 257)] 
#Not really 2 regions, just historic sample
```
...
which(discrepancy == TRUE)
#CloneIndex 45 - spotted above, direct capture agrees with majority
table(smd$knownSEX[which(smd$finalClone == 45)])
#CloneIndex 144 - spotted previously, direct captured cub
table(smd$knownSEX[which(smd$finalClone == 144)])
#CloneIndex 298 - spotted above, direct capture agrees with majority
table(smd$knownSEX[which(smd$finalClone == 298)])
...
# 63-DC: den check cub, called a female by GT, M in the field
# 497-24: capture in the matches, a F
# 212-23 & 271-23: no captures, but majority F
# 774-24: capture in the matches, F
# 5-23: majority M no captures in matches
# 655-23: majority M no captures in matches
