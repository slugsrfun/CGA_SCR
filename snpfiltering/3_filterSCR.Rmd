...
We also remove loci that are missing data for more than 50% of samples. This reduces the mean proportion of missing genotypes across samples substantially, compared to more relaxed missing data filters. Note that the `--max-missing` argument to `vcftools` is defined such that "0 allows sites that are completely missing and 1 indicates no missing data allowed" (see the [`vcftools` manual](https://vcftools.sourceforge.net/man_latest.html)).  

Prior to these filters, we have `r snpct` loci in the VCF...  
```{r engine="bash", comment = ""}
#Max missing is 30% of samples (per locus)
vcftools --vcf filt3.recode.vcf --out filt4 \
            --max-missing 0.5 --remove-filtered-all --recode
```
