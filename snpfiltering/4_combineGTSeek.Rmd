...
**ADD individual level filters for heterozygosity (Fis < -0.5) and missing data (50% for LGC, 100 SNPs for GTS)**.  
**ADD pID and pIDsibs calculations**.  
**FIX NSNPGT - MAKE A NEW COLUMN FOR NSNPCOMBO OR SOMETHING - UPDATE COUNTS AFTER COMBINING THE DATASETS!!**
**Look back at Leftovers script (in Desktop/SCRms_revs/) for useful code for pID, etc.**
...
We'll walk through these steps below. At the end of the markdown, we include a short section evaluating the power of the marker set via calculation of probabilities of identity for random individuals ($p_{\text{ID}}$) and for siblings ($p_{\text{IDsibs}}$). These measure the chance that two unrelated individuals, or a pair of siblings, would share a genotype at all markers in the panel.  
