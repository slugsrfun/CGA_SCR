...
We'll walk through these steps below. At the end of the markdown, we include a short section evaluating the power of the marker set via calculation of probabilities of identity for random individuals ($p_{\text{ID}}$) and for siblings ($p_{\text{IDsib}}$). These measure the chance that two unrelated individuals, or a pair of siblings, would share identical genotypes.  
...
$$p_{\text{ID}}=\sum(p_i^4)+(2p_ip_j)^2$$

Then the power of the combined marker set is the product of $p_{\text{ID}}$ across all SNP loci, as markers are assumed to provide independent information on identity.  

First, extract allele frequencies for all loci in the combined dataset.  
```{r engine="bash", comment=""}
vcftools --vcf finalSCR.recode.vcf --out finalSCR \
          --freq
```

Now use these to calculate $p_{\text{ID}}$.  
```{r pID}
freqs <- read.table("finalSCR.frq", header = FALSE)
...
A more conservative estimate of the power of the set is provided by $p_{\text{IDsib}}$, which measures the same probability for siblings. Also from [Waits et al. (2001)](https://doi.org/10.1046/j.1365-294X.2001.01185.x)...  

$$p_{\text{IDsib}} = 0.25 + \left( 0.5 \sum p_i^2\right)+ \left [ 0.5 \left( \sum p_i^2\right)^2 \right ] - \left( 0.25 \sum p_i^4\right)$$

As above, the $p_{\text{IDsib}}$ for the dataset is the product across loci.  
```{r pIDsib}
...
for(l in 1:nrow(freqs)) {
  freqs$pIDsib[l] <- 0.25 + (0.5*sum(c(freqs$FRQ[l]^2,(1-freqs$FRQ[l])^2))) +
                      ...
                      (0.25*sum(c(freqs$FRQ[k]^4, (1-freqs$FRQ[l])^4)))
}
...
PIDsibsub <- matrix(data = NA, nrow = 100, ncol = length(subsize))
...
for(size in 1:length(subsize)) {
  for(rep in 1:100) {
    ...
    pIDsub[rep, size] <- (freqs$FRQ[l]^4+(1-freqs$FRQ[l])^4) + 
                          (2*freqs$FRQ[l]*(1-freqs$FRQ[l]))^2
    pIDsibsub[rep, size] <-  0.25 + 
                          (0.5*sum(c(freqs$FRQ[l]^2,(1-freqs$FRQ[l])^2))) +
                          (0.5*(sum(c(freqs$FRQ[l]^2, (1-freqs$FRQ[l])^2)))^2) - 
                          (0.25*sum(c(freqs$FRQ[k]^4, (1-freqs$FRQ[l])^4)))
...
#Plot the results
library(scales)
plot(subsize, colMeans(log10(pIDsub)), type = "l", 
     ylim = range(log10(pIDsub)), lwd = 2, col = "black", 
     xlab = "Number of SNP loci", ylab = "Probability of Identity")
quant95 <- apply(log10(pIDsub), 1, quantile, prob = c(0.025, 0.975))
polygon(x = c(subsize, subsize[length(subsize):1]),
        y=c(quant95[1,], quant95[2,][ncol(quant95):1]),
        col=alpha("black", 0.1), border = NA)

library(scales)
plot(subsize, colMeans(log10(pIDsibsub)), type = "l", 
     ylim = range(log10(pIDsibsub)), lwd = 2, col = "black", 
     xlab = "Number of SNP loci", ylab = "Probability of Identity - Siblings")
quant95 <- apply(log10(pIDsibsub), 1, quantile, prob = c(0.025, 0.975))
polygon(x = c(subsize, subsize[length(subsize):1]),
        y=c(quant95[1,], quant95[2,][ncol(quant95):1]),
        col=alpha("black", 0.1), border = NA)
```
